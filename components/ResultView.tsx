...
const REPORT_ELEMENT_ID = "diagnosis-report";
...
export default function ResultView({
  ...
  async function handleDownloadPdf() {
    ...
    try {
      const safeName = firstName.toLowerCase().replace(/[^a-z0-9]+/gi, "-");
      await downloadDiagnosisPdf(REPORT_ELEMENT_ID, `diagnostico-lcd-${safeName || "resultado"}.pdf`);
    } catch (err) {
  ...
  return (
    <div className="df-fadeup">
      {/* Botão de download em destaque, fora da área capturada no PDF */}
      <div className="mb-8">
        <button
          type="button"
          onClick={handleDownloadPdf}
          disabled={generatingPdf}
          className="inline-flex items-center gap-2 bg-brand hover:bg-brand-hover disabled:opacity-60 text-white font-bold text-sm tracking-wide uppercase px-7 py-4 rounded-xl2 shadow-[0_8px_24px_-8px_rgba(196,29,51,0.55)] transition-colors"
        >
          <span aria-hidden>⬇</span>
          {generatingPdf ? "Gerando PDF..." : "Baixar PDF do diagnóstico"}
        </button>
        {pdfError && <p className="text-xs text-accent-neg mt-2">{pdfError}</p>}
      </div>

      <div id={REPORT_ELEMENT_ID} className="bg-bg">
        ...
        <div className="card p-6 mb-8">
          ...
          <span
            className="mono-num block text-[56px] font-black leading-none"
        ...
        {secondInsight && secondWeakest && (
          <div className="card p-6 mb-8">
            ...
            <p className="text-body text-[14px] leading-relaxed">{secondInsight.consequence}</p>
          </div>
        )}
        ...
        <a
          ...
          className="inline-block bg-[#16a34a] hover:brightness-95 text-white font-semibold text-sm tracking-wide uppercase px-8 py-4 rounded-xl2 transition-all mb-10"
        >
          Falar com nosso time no WhatsApp →
        </a>

        {/* DISCLAIMER */}
        <p className="text-[11px] leading-relaxed text-muted border-t border-border pt-5">
